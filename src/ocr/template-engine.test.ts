...
describe('TemplateEngine', () => {
    ...
    const segments: Segment[] = [
      {
        ...
        width: 20,
        height: 40,
        imageData: createRgbaImageDataFromGreyscale(createPattern(2048, 255)),
      }, // Match '2'
      {
        x: 10,
        y: 0,
        width: 20,
        height: 40,
        imageData: createRgbaImageDataFromGreyscale(createPattern(2048, 0)),
      }, // Match '0'
    ];
  ...
  it('deve retornar um caractere "?" se nenhum template exceder o limite mínimo (Threshold)', () => {
    ...
    engine = new TemplateEngine(
      { '0': createPattern(2048, 0) } as any,
      0.95 /* threshold muito alto */,
    );
